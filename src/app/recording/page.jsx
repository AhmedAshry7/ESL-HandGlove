...
function LiveVoltageMonitor({ voltages, sensorHealth, knotsByAxis }) {
  const channelKnots = buildChannelKnots(knotsByAxis);
  ...
  return (
    <div style={{ background: 'rgba(10,12,28,0.98)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, overflow: 'hidden', backdropFilter: 'blur(12px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: '#a0aec0', letterSpacing: '0.8px', textTransform: 'uppercase' }}>Hall Sensor Levels (0-100)</span>
        <span style={{ fontSize: 10, color: '#4a5568' }}>calibrated percent</span>
      </div>
      <div style={{ padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: 5 }}>
        {CH_LABELS.map((label, idx) => {
          ...
          const pct = percentFromKnots(v, channelKnots[idx]);
          const pctValue = pct === null ? null : Math.round(pct * 100);
          const fill = pctValue ?? 0;
          const color = pctValue === null ? '#4a5568' : percentToColor(pctValue);
            ...
            <div style={s.panel}>
              ...
              {calTab === 'voltages' && (
                <div style={{ marginTop: 12 }}>
                  <LiveVoltageMonitor voltages={rawVoltages} sensorHealth={sensorHealth} knotsByAxis={knotsByAxis} />
                </div>
              )}
...
const s = {
  ...
  body: { flex:1, display:'flex', gap:32, padding:32, maxWidth:1800, margin:'0 auto', width:'100%' },
  leftCol: { flex:1, display:'flex', flexDirection:'column', gap:20, minWidth:0 },
  rightCol: { width:400, flexShrink:0, display:'flex', flexDirection:'column', gap:16 },
